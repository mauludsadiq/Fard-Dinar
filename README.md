...
1,403 lines of FARD implement the full engine plus wallet and vendor surfaces that never existed in Rust. The only Rust is the AHD-1024 binary.
...
**Typical single-node flow**

    # Terminal 1 — registry
    fardrun run --program fard/bin/fd_registry.fard --out ./out -- \
      --watch ./registry_events --registry-out registry.json --bind 127.0.0.1:7371

    # Terminal 2 — node
    fardrun run --program fard/bin/fd_node.fard --out ./out -- \
      --watch ./node_events --genesis examples/genesis_rewards.json \
    ...
    # Poll registry then copy canonical events to node
    curl -X POST http://127.0.0.1:7371/v1/poll
    cp registry_events/*.json node_events/
...
1,403 lines of FARD cover the full engine plus wallet and vendor surfaces that were never in the Rust codebase. The only Rust remaining is the AHD-1024 hash binary.
