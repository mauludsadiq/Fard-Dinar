...
Registries sync by fetching each other's canonical registry state during poll.
...
**Event propagation:**
- After merging, the registry's watch directory contains all locally submitted events
- Events from peers are not automatically written to the local watch directory
- Operators copy canonical event files between registries or use the ingest endpoint to submit them directly
